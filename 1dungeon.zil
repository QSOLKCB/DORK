"1DUNGEON for
    ...
    Based on Zork I by Infocom and thoroughly polluted with cursed memes.
    © 2025 QSOL‑IMC.  All rights reserved.  Released under the MIT license."

<DIRECTIONS NORTH EAST WEST SOUTH NE NW SE SW UP DOWN IN OUT LAND>

; In DORK we collect clout rather than mere score.  Increase the max to a suitably
; internet‑worthy number.
<GLOBAL SCORE-MAX 420>
; <GLOBAL CLOUT-MAX 420> ; uncomment to track clout separately if desired.
...
; ---------------------------------------------------------------------------
; Meme artifacts unique to DORK.  These objects live in the living room when
; the game begins and serve no practical purpose beyond humor.
; ---------------------------------------------------------------------------

<OBJECT QUOTE-TWEET-AMULET
    (IN LIVING-ROOM)
    (SYNONYM AMULET QUOTE TWEET)
    (DESC "enraged quote‑tweet amulet")
    (FLAGS TAKEBIT)
    (TEXT
 "This shiny amulet pulses with chaotic energy.  When worn it compels you to
 quote‑tweet the worst takes imaginable.")>

<OBJECT COPYPASTA-TABLET
    (IN LIVING-ROOM)
    (SYNONYM TABLET COPYPASTA COPYP ASTA)
    (DESC "ancient copypasta tablet")
    (FLAGS TAKEBIT READBIT)
    (TEXT
 "An ancient stone tablet covered in barely legible ASCII text.  It appears
 to be a relic from an era when \"rage faces\" ruled the land.")>

<OBJECT NFT-ROCK
    (IN LIVING-ROOM)
    (SYNONYM NFT ROCK JPEG)
    (DESC "NFT of a JPEG of a rock")
    (FLAGS TAKEBIT)
    (TEXT
 "A certificate proving ownership of a low‑resolution image of a rock.  It
 emits a faint smell of burnt GPU and bad investments.")>

<OBJECT BRAND-PALADIN
    (IN LIVING-ROOM)
    (SYNONYM PALADIN BRAND INTERN)
    (DESC "brand intern paladin")
    (FLAGS NDESCBIT ACTORBIT)
    (ACTION BRAND-PALADIN-F)>

; Routine for interacting with the brand paladin.  He mostly reacts with
; corporate enthusiasm.
<ROUTINE BRAND-PALADIN-F ()
    <COND (<VERB? EXAMINE FIND>
           <TELL
"Clad in ill‑fitting armor adorned with logos, this paladin is sworn to
 uphold brand guidelines above all else." CR>)
          (<VERB? TALK TELL HELLO>
           <TELL
\"Remember to like and subscribe!\" the paladin intones with forced
 cheerfulness.  He doesn't seem particularly threatening." CR>)
          (T
           <TELL "The brand paladin stares blankly, awaiting his next meme assignment." CR>)>>
